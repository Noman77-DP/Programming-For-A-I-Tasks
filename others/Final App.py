...
def apply_styles(root):
    ...
    # Watermark style - BOLD, BIG, BLACK
    style.configure('Watermark.TLabel', background=BG, foreground='#000000', font=(FONT_FAMILY, 12, 'bold'))
...
def main_application_window():
    ...
    user_label.pack(side='right', padx=(0, 12))
    ...
    logout_btn = ttk.Button(topbar, text="Logout", command=logout_action, style='Danger.TButton')
    ...
    location_tab = ttk.Frame(notebook, padding=12, style='Tab.TFrame')
    ...
    notebook.add(location_tab, text='Asset Locations')
    ...
    setup_location_tab(location_tab)
    ...
    # Watermark (bottom-right) - BOLD, BIG, BLACK
    watermark = ttk.Label(root, text="Made By NOMAN (24F-AI-202)", style='Watermark.TLabel')
...
def setup_assets_tab(tab_frame):
    ...
    def populate_assets_tree(rows, tree):
        ...
        for idx, r in enumerate(rows):
            ...
            # Remove location from display (index 7) - keep only indices 0-6
            display_values = values[:7]
    ...
    tree = ttk.Treeview(table_container, columns=cols, show='headings', selectmode='browse', height=15)
    for c in cols:
    ...
    tree.column("Purchase Date", width=100, anchor='center')
    ...
    def apply_filters_local():
        ...
        for r in assets_table_rows:
            ...
            if dfilter != "All" and (dept_name is None or dept_name != dfilter):
                continue
            if q:
                searchable = " ".join([str(name or ""), str(category or ""), str(dept_name or ""), str(format_asset_code(asset_id))]).lower()
    ...
    def on_asset_select(event):
        ...
        # Get location from the raw data
        for raw_row in assets_table_rows:
            if raw_row[0] == GLOBAL_SELECTED_ASSET_ID:
                location_var.set(raw_row[7] if raw_row[7] else "")
                break
    ...
    def download_csv():
        filepath = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV", "*.csv")], initialfile="assets_inventory.csv")
        if not filepath:
            return
        rows = []
    ...
    def generate_receipt_with_qr_local():
        ...
        # Get location from raw data
        location = ""
        for raw_row in assets_table_rows:
            if format_asset_code(raw_row[0]) == asset_code:
                location = raw_row[7] if raw_row[7] else ""
                break
        ...
        draw.text((28, y), f"Department: {dept}", font=font_label, fill='black'); y += spacing
        draw.text((28, y), f"Location: {location}", font=font_label, fill='black')
...
def setup_employees_tab(tab_frame):
    ...
    def export_employees_csv():
        filepath = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV", "*.csv")], initialfile="employees.csv")
        if not filepath:
            return
        rows = []
        for iid in tree.get_children():
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["ID", "Name", "Department", "Job Title", "Email", "Phone", "Joined Date"])
                for r in rows:
                    writer.writerow(r)
            messagebox.showinfo("Saved", f"CSV saved to {filepath}")
        except Exception as e:
    ...
    header_actions.grid(row=0, column=1, sticky='ne', padx=5, pady=5)
    ttk.Button(header_actions, text="Export CSV", command=export_employees_csv, style='TButton').pack(side='left', padx=2)
    ttk.Button(header_actions, text="Refresh", command=refresh_employees_view, style='Clear.TButton').pack(side='left', padx=2)
...
def setup_assignments_tab(tab_frame):
    ...
    def export_assignments_csv():
        filepath = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV", "*.csv")], initialfile="assignments.csv")
        if not filepath:
            return
        rows = []
        for iid in tree.get_children():
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["ID", "Asset", "Employee", "Assigned Date", "Return Date"])
                for r in rows:
                    writer.writerow(r)
            messagebox.showinfo("Saved", f"CSV saved to {filepath}")
        except Exception as e:
    ...
    header_actions = ttk.Frame(header_frame)
    header_actions.grid(row=0, column=1, sticky='ne', padx=5, pady=5)
    ttk.Button(header_actions, text="Export CSV", command=export_assignments_csv, style='TButton').pack(side='left', padx=2)
    ttk.Button(header_actions, text="Refresh", command=lambda: refresh_assignment_comboboxes(tree), style='Clear.TButton').pack(side='left', padx=2)
...
def setup_maintenance_tab(tab_frame):
    ...
    def export_maintenance_csv():
        filepath = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV", "*.csv")], initialfile="maintenance.csv")
        if not filepath:
            return
        rows = []
        for iid in tree.get_children():
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["ID", "Asset", "Vendor", "Issue", "Start Date", "End Date", "Cost"])
                for r in rows:
                    writer.writerow(r)
            messagebox.showinfo("Saved", f"CSV saved to {filepath}")
        except Exception as e:
    ...
    header_actions = ttk.Frame(header_frame)
    header_actions.grid(row=0, column=1, sticky='ne', padx=5, pady=5)
    ttk.Button(header_actions, text="Export CSV", command=export_maintenance_csv, style='TButton').pack(side='left', padx=2)
    ttk.Button(header_actions, text="Refresh", command=refresh_maintenance_views, style='Clear.TButton').pack(side='left', padx=2)
...
def setup_reports_tab(tab_frame):
    ...
    def export_audit_csv():
        filepath = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV", "*.csv")], initialfile="audit_log.csv")
        if not filepath:
            return
        rows = []
        for iid in tree.get_children():
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["ID", "Timestamp", "Asset ID", "Action", "Performed By", "Notes"])
                for r in rows:
                    writer.writerow(r)
            messagebox.showinfo("Saved", f"CSV saved to {filepath}")
        except Exception as e:
    ...
    header_actions = ttk.Frame(header_frame)
    header_actions.grid(row=0, column=1, sticky='ne', padx=5, pady=5)
    ttk.Button(header_actions, text="Export CSV", command=export_audit_csv, style='TButton').pack(side='left', padx=2)
    ttk.Button(header_actions, text="Refresh", command=refresh_log_view, style='Clear.TButton').pack(side='left', padx=2)
...
# --- ASSET LOCATIONS TAB (UPDATED - Shows detailed asset information) ---
def setup_location_tab(tab_frame):
    tab_frame.configure(style='Tab.TFrame')
    tab_frame.columnconfigure(0, weight=1)
    tab_frame.rowconfigure(1, weight=1)

    def fetch_locations_table(tree_view):
        """Fetch all non-archived assets with detailed information grouped by location."""
        tree_view.delete(*tree_view.get_children())
        cursor = conn.cursor()
        try:
            cursor.execute("""
                SELECT 
                    a.asset_id, a.name, a.category, a.status,
                    IFNULL(a.location, 'No Location') as location,
                    d.dept_name
                FROM Assets a
                LEFT JOIN Departments d ON a.department_id = d.dept_id
                WHERE a.status != 'Archived'
                ORDER BY IFNULL(a.location, 'No Location'), a.asset_id
            """)
            rows = cursor.fetchall()
            for r in rows:
                asset_code = format_asset_code(r[0])
                display_row = (asset_code, r[1], r[2], r[3], r[4], r[5] or "")
                tree_view.insert("", "end", values=display_row)
        except mysql.connector.Error as e:
            messagebox.showerror("DB Error", f"Failed to fetch asset locations: {e}")

    def export_locations_csv():
        filepath = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV", "*.csv")], initialfile="asset_locations_detailed.csv")
        if not filepath:
            return
        rows = []
        for iid in tree.get_children():
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["Asset ID", "Name", "Category", "Status", "Location", "Department"])
                for r in rows:
                    writer.writerow(r)
            messagebox.showinfo("Saved", f"CSV saved to {filepath}")
        except Exception as e:
    ...
    def refresh_locations_view():
        fetch_locations_table(tree)

    header_frame = ttk.Frame(tab_frame)
    header_frame.grid(row=0, column=0, sticky='ew')
    header_frame.columnconfigure(0, weight=1)

    ttk.Label(header_frame, text="Asset Locations (All Details)", style='SubHeader.TLabel').grid(row=0, column=0, sticky='w', pady=5, padx=5)

    header_actions = ttk.Frame(header_frame)
    header_actions.grid(row=0, column=1, sticky='ne', padx=5, pady=5)
    ttk.Button(header_actions, text="Export CSV", command=export_locations_csv, style='TButton').pack(side='left', padx=2)
    ttk.Button(header_actions, text="Refresh", command=refresh_locations_view, style='Clear.TButton').pack(side='left', padx=2)
    ...
    columns = ("Asset ID", "Name", "Category", "Status", "Location", "Department")
    tree = ttk.Treeview(table_container, columns=columns, show="headings")
